...
export function getCachedChannels() {
    return readJson(CACHE_FILE, []) === [] ? [] : Object.values(readJson(CACHE_FILE, {}));
}
...
// ─────────────────────────────────────────────
//  FIX #10 — Universal target resolver.
//  Single implementation shared by schedule.js,
//  profile.js, etc. Replaces the 4× duplicated
//  resolveTarget copies.
//  Returns { jid, type } or null.
// ─────────────────────────────────────────────
export async function resolveTargetUniversal(sock, raw) {
    if (!raw) return null;
    ...
    if (clean.includes('@') && !clean.startsWith('@')) {
        ...
        if (clean.endsWith('@lid')) {
            try {
                ...
                if (pn) return { jid: stripDevice(pn), type: 'user' };
            } catch {}
    ...
    if (clean.startsWith('@')) {
        ...
        if (r.pn) return { jid: stripDevice(r.pn), type: 'user' };
        return null;
    }
    ...
    if (digits.length >= 7) {
        ...
        if (r.pn) return { jid: r.pn, type: 'user' };
    }
