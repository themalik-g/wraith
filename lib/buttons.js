...
// Powered by @qadeerxtech/qadeer-btns
// ─────────────────────────────────────────────
  ...
  if (m.interactiveResponseMessage) {
    ...
    if (flowReply) {
      try {
        ...
        if (params.reference_id) return params.reference_id;
      } catch {}
    ...
    if (b && typeof b === 'object') {
      if (b.type) {
        switch (b.type) {
          ...
          case 'select':
          case 'list': return createSingleSelect(b.title, b.sections);
          case 'location': return createLocationRequest(b.text || b.display_text || 'Share Location');
          default: break;
        }
      }
      if (b.id && (b.text || b.display_text)) {
        return createQuickReply(b.text || b.display_text, b.id);
      }
    }
    return b;
  });
 ...
 * @param {Object} options - Message parameters ({ body, footer, header, buttons, image, video, document })
...
export async function sendInteractive(sock, jid, options = {}, opts = {}) {
  ...
  // 1. Primary engine: @qadeerxtech/qadeer-btns with biz / native_flow / bot node injection
  try {
    if (qadeerBtns && typeof qadeerBtns.sendInteractiveMessage === 'function') {
      const qadeerContent = {
        ...
        title: header,
        footer: footer,
        interactiveButtons: normalizedButtons,
    ...
    try {
      console.warn('[sendInteractive] qadeer-btns failed, trying native relay fallback:', e?.message || e);
    } catch {}
  }

  // 2. Secondary fallback: Native Baileys relay Message with additionalNodes injection
  try {
    ...
    try {
      console.warn('[sendInteractive] native relay failed, trying gifted-btns fallback:', e?.message || e);
    } catch {}
  }

  // 3. Tertiary fallback: gifted-btns wrapper
  try {
  ...
  // 4. Fallback to standard text message if interactive sending fails
  return sendFallbackText(sock, jid, options, opts);
}
