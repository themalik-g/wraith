...
async function convertBufferToMp3(inputBuffer, bitrate = 192) {
    ...
    const tmpIn = path.join(os.tmpdir(), `wraith_audio_in_${Date.now()}_${Math.random().toString(36).slice(2)}.tmp`);
    const tmpOut = path.join(os.tmpdir(), `wraith_audio_out_${Date.now()}_${Math.random().toString(36).slice(2)}.mp3`);
