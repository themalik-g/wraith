...
//
//  · Every source has a hard timeout — hung sources are skipped.
//  · Every result is normalised to MP3 (in memory, via ffmpeg pipes).
//  · No files are left on disk; buffers live only in RAM.
// ─────────────────────────────────────────────
...
const MP3_CONVERT_TIMEOUT = 60_000;
...
async function ensureMp3(result, onStatus) {
    ...
    return { ...result, buffer: converted, size: converted.length, format: 'mp3', mimetype: 'audio/mpeg' };
}
...
export async function fetchFromSoundCloud(query, maxBytes, onStatus) {
    ...
    try {
        ...
        return {
            buffer, title: result.title || query, artist: '', artwork: null,
            provider: 'soundcloud (full)', size: buffer.length,
            format: 'mp3', mimetype: 'audio/mpeg', preview: false,
        };
    } finally {
...
export async function fetchFromSoundCloudScdl(query, maxBytes, onStatus) {
    ...
    if (onStatus) onStatus('🎵 SoundCloud (scdl): searching…');
    ...
    const best = pickBest(streamable, query, (t) => ({
        title: t.title, artist: t.user?.username, duration: (t.duration || 0) / 1000,
    }));
    ...
    return {
        buffer, title: best.title, artist: best.user?.username || '',
        artwork: best.artwork_url || null, provider: 'soundcloud (scdl)',
        size: buffer.length, format: fmt.ext, mimetype: fmt.mime, preview: false,
    };
}

// ── TIER 3: Apple Music FULL ──
export async function fetchFromAppleMusic(query, maxBytes, onStatus) {
    ...
    const best = pickBest(songs, query, (t) => ({ title: t.title, artist: t.artists, duration: 0 }));
    ...
    return {
        buffer, title: dl.title || best.title, artist: dl.artists || best.artists || '',
        artwork: dl.image || best.cover || null, provider: 'apple-music (full)',
        size: buffer.length, format: fmt.ext, mimetype: fmt.mime, preview: false,
    };
}
...
export async function fetchFromDeezer(query, maxBytes, onStatus) {
    ...
    const best = pickBest(data, query, (t) => ({
        title: t.title, artist: t.artist?.name, duration: t.duration,
    }));
    ...
    return {
        buffer, title: best.title, artist: best.artist?.name || '',
        artwork: best.album?.cover_xl || best.album?.cover_big || null,
        provider: 'deezer (full)', size: buffer.length,
        format: 'mp3', mimetype: 'audio/mpeg', preview: false,
    };
}
...
export async function fetchAudioFromAnySource(query, maxBytes, onStatus) {
    const errors = [];
    const attempts = [
        ['sc-ytdlp', fetchFromSoundCloud,     TIMEOUTS['sc-ytdlp']],
        ['scdl',     fetchFromSoundCloudScdl, TIMEOUTS.scdl],
        ['apple',    fetchFromAppleMusic,     TIMEOUTS.apple],
        ['deezer',   fetchFromDeezer,         TIMEOUTS.deezer],
    ];

    for (const [name, fn, timeout] of attempts) {
        try {
            const raw = await withTimeout(fn(query, maxBytes, onStatus), timeout, name);
            const result = await ensureMp3(raw, onStatus);
            if (onStatus) onStatus(`✅ ${result.provider}: ${result.title}`);
            return result;
        } catch (e) {
