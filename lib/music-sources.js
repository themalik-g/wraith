...
// Fuzzy matching: scores results by title/artist/duration
// ─────────────────────────────────────────────
...
function scoreTrack({ title, artist, duration }, query) {
    ...
    // Duration preference: 2–7 min is ideal
    ...
    // Penalise covers / remixes / live / instrumentals
    const bad = [
...
// ─── Pick the best result from a list ───
...
export async function fetchFromSoundCloud(query, maxBytes, onStatus) {
    ...
    const best = pickBest(streamable, query, (t) => ({
        ...
        duration: (t.duration || 0) / 1000, // ms → s
    }));
    ...
    if (onStatus) {
        onStatus(`✅ SoundCloud complete (${(buffer.length / 1048576).toFixed(1)} MB)`);
    }
...
export async function fetchFromAppleMusic(query, maxBytes, onStatus) {
    ...
    if (onStatus) {
        onStatus(`✅ Apple Music preview complete (${(buffer.length / 1048576).toFixed(1)} MB)`);
    }
...
export async function fetchFromDeezer(query, maxBytes, onStatus) {
    ...
    if (onStatus) {
        onStatus(`✅ Deezer preview complete (${(buffer.length / 1048576).toFixed(1)} MB)`);
    }
