...
function ensurePptxGenJS() {
  try {
    ...
    console.log('[ppt] pptxgenjs not found. Installing on-demand...');
    try {
      execSync('npm install --no-save pptxgenjs@^3.12.0', { stdio: 'inherit' });
      const mod = require('pptxgenjs');
      return mod?.default || mod;
    } catch (err) {
      throw new Error(`Failed to install pptxgenjs on-demand: ${err.message}`);
    }
  }
}
...
export async function renderPPTBuffer(slideData) {
  const PptxGenJS = ensurePptxGenJS();
