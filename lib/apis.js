...
// ── BOOKS ───────────────────────────────────────────────────────────────────
// ★ FIX: resolve a REAL archive.org PDF file (the old {ia}.pdf guess 404'd)
async function resolveArchivePdf(ia) {
  try {
    const meta = await httpGetJson(`https://archive.org/metadata/${ia}`, { timeout: 10000 });
    const files = meta?.files || [];
    const pdf = files.find((f) => /\.pdf$/i.test(f.name) && !/scandata|djvu|text|epub/i.test(f.name))
  ...
  if (r.source.includes('gutendex')) {
    for (const b of (r.data.results || []).slice(0, limit)) {
      ...
      books.push({
        ...
        year: null, // ★ FIX: gutendex carries no publish year (old code was always-null anyway)
    ...
    for (const d of (r.data.docs || []).slice(0, limit)) {
      ...
      let direct = null;
      if (ia) direct = await resolveArchivePdf(ia); // ★ FIX: real file URL
      books.push({
        ...
        downloadCount: d.ia?.length ? 1 : 0,
      });
    }
  }
  return { ok: true, books, source: r.source.includes('gutendex') ? 'Project Gutenberg' : 'Open Library' };
}
...
export async function searchSong(query) {
  ...
  return { ok: true, title: rec.title, artist: artistCredit, album: rec.releases?.[0]?.title || '—', duration: rec.length ? Math.round(rec.length / 1000) : 0, cover: rec.releases?.[0]?.id ? `https://coverartarchive.org/release/${rec.releases[0].id}/front-250` : null, preview: null, source: 'MusicBrainz' };
}
...
// ── STOCK IMAGES ★ FIXED ────────────────────────────────────────────────────
// Primary: Openverse (keyless, CC-licensed). Fallback: LoremFlickr (keyless).
export async function searchImages(query, count = 5) {
  try {
    const r = await httpGetJson(`https://api.openverse.org/v1/images/?q=${encodeURIComponent(query)}&page_size=${count}`, { timeout: 15000 });
  ...
  // Fallback: deterministic-random CC images per query (no JSON API needed)
  const images = [];
...
// ── CURRENCY ★ FIXED — now supports PKR/NPR/BDT/LKR etc. ────────────────────
