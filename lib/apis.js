...
// ── BOOKS — Gutenberg priority, downloadable-only ──
...
async function resolveArchivePdf(ia) {
  try {
    ...
    // ★ Skip lending-restricted items — these return 401 on download
    if (meta?.metadata?.['access-restricted-item'] === 'true') return null;

    const files = meta?.files || [];
    const pdf =
      files.find((f) => /\.pdf$/i.test(f.name) && !/scandata|djvu|text|epub|_bw\.|_text/i.test(f.name) && f.private !== 'true') ||
...
export async function searchBooks(query, limit = 8) {
  // ── Tier 1: Project Gutenberg (always downloadable) ──
  try {
    ...
    if (r?.results?.length) {
      ...
      for (const b of r.results.slice(0, limit)) {
        ...
        if (!url) continue; // skip non-downloadable
        books.push({
          id: String(b.id),
          title: b.title,
    ...
    console.warn('[books] Gutenberg failed:', e.message);
  }

  // ── Tier 2: Open Library / archive.org (downloadable only) ──
  let openLibRes;
  try {
    openLibRes = await httpGetJson(
      `https://openlibrary.org/search.json?q=${encodeURIComponent(query)}&limit=${limit * 2}`,
      { timeout: 12000 }
    );
  } catch (e) {
    console.warn('[books] Open Library failed:', e.message);
  ...
  for (const d of docs) {
    ...
    const direct = await resolveArchivePdf(ia);
    if (!direct) continue; // skip non-downloadable
  ...
  return { ok: true, books, source: 'Open Library (archive.org)' };
}
// ── MOVIE ───────────────────────────────────────────────────────────────────
