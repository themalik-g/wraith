...
| ⬇️ **Media Downloader** | Integrated `@postfetch/core` + `yt-dlp` + `ytdlp-nodejs` engine (`.dl`, `.play`, `.ytv`, `.ytdl`, `.mp3`, `.pdl`, `.pdlzip`) for YouTube video/audio (up to 400MB cap), audio extraction, and image carousels (Instagram `/p/`, TikTok `/photo/`, Pinterest, Twitter, Facebook). |
