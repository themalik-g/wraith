...
| ⬇️ **Media Downloader** | Integrated `@postfetch/core` + `yt-dlp` + `ytdlp-nodejs` engine (`.dl`, `.play`, `.ytv`, `.video`, `.ytdl`, `.mp3`, `.pdl`, `.pdlzip`) for YouTube video/audio (up to 400MB cap, SD 360p/480p WhatsApp playable format), audio extraction, and image carousels (Instagram `/p/`, TikTok `/photo/`, Pinterest, Twitter, Facebook). |
| 📦 **File & Social Downloader** | GitHub repo downloader (`.gitdl`), MediaFire downloader (`.mfdl`), social profile search & media downloading (`.ig`, `.tiktok`, `.fb`), song downloader (`.song`, `.play`). |
...
| 👥 **Admin & Group** | Group management (`.open`, `.close`, `.kick`, `.add`, `.promote`, `.demote`, `.approveall`, `.declineall`, `.kickall`, `.kickcc`, `.tagall`, `.hidetag`, `.setgpp`, `.setgdesc`, `.mute`, `.unmute`, `.welcome`, `.goodbye`), chat disappearing messages (`.disappearing`), plus protection (`.antilink`, `.antispam`, `.antisticker`, `.rejectcalls`). |
